...
export function PathResult({ result }: PathResultProps) {
  if (!result) {
    return (
      <div className="bg-white rounded-3xl border border-stone-100 p-8 md:p-10 shadow-[0_20px_50px_rgb(0,0,0,0.05)] mt-12 relative overflow-hidden flex flex-col items-center justify-center min-h-[320px]">
        {/* Góc hoa văn trang trí */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-[#FAF9F6] rounded-bl-full -z-10 border-l border-b border-stone-50"></div>
        <div className="text-stone-300 mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="m12 8 4 4-4 4"/></svg>
        </div>
        <p className="text-stone-400 italic text-sm">Bản đồ kết nối sẽ hiển thị tại đây.</p>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-3xl border border-stone-100 p-8 md:p-10 shadow-[0_20px_50px_rgb(0,0,0,0.05)] animate-fade-in mt-12 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-4">
        <h3 className="text-2xl font-[family-name:var(--font-playfair)] font-bold text-stone-800 flex items-center gap-3">
          Sự Kết Nối Hoàn Hảo
        </h3>
      ...
      <div className="relative pt-4 pb-8">
        {/* Đường kẻ nối ngang */}
        <div className="absolute top-1/2 left-0 right-0 h-px bg-stone-200 -translate-y-1/2 z-0 hidden md:block"></div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10 overflow-x-auto custom-scrollbar px-2 py-6 -my-6">
          {result.path.map((node, index) => (
            <div key={index} className="flex flex-col items-center group shrink-0">
              {/* Thẻ Card Tên người */}
              <div
                className={`
                ...
                ${
                  index === 0
                    ? "bg-[#FDFBF7] border-2 border-[#E8C8C8] text-[#B87C7C]"
                    : index === result.path.length - 1
                    ? "bg-[#D08B8B] border-2 border-[#D08B8B] text-white shadow-md shadow-[#D08B8B]/20"
                    : "bg-white border border-stone-200 text-stone-600 group-hover:border-[#E8C8C8] group-hover:text-stone-800"
                }
              `}
                style={{
